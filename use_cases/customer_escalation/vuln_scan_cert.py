...
def main(input_file_path, output_file_path, output_text_file_path):
    # Read input CSV file
    ...
    with open(output_text_file_path, 'w') as output_text_file:
        for index, row in data.iterrows():
            ...
            output_text_file.write(f"Severity: {severity}\n")
            ...
            output_text_file.write(f"RHSAs: {', '.join(rhsa_list) if rhsa_list else 'N/A'}\n")
            ...
            for advisory_product in advisories_products:
                ...
                output_text_file.write(f"Advisory: {advisory_product['advisory']}\n")
                output_text_file.write(f"Product Name: {advisory_product['product_name']}\n")
                output_text_file.write(f"Package: {advisory_product['package']}\n")
                output_text_file.write(f"Version: {advisory_product['version']}\n")
                output_text_file.write(f"Advisory URL: {get_advisory_url(advisory_product['advisory'])}\n")
            ...
            output_text_file.write(f"Fixed Status: {affected_status[0] if affected_status else 'N/A'}\n")
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python vuln_scan_cert.py <input_file_path> <output_file_path> <output_text_file_path>")
        sys.exit(1)

    input_file_path = sys.argv[1]
    output_file_path = sys.argv[2]
    output_text_file_path = sys.argv[3]
    ...
    main(input_file_path, output_file_path, output_text_file_path)

# command on terminal: python vuln_scan_cert.py "Vuln Scan Cert Test Data 1 - Resubmitted Python.csv" "output - Sheet1.csv" "output.txt"
